...
The 2014-2019 laser diode sales data will be split into two datasets: visible LDs and NIR laser diodes.

The full distribution of each dataset is left skewed. However, viewing the full distribution means we also see the outliers, which stretch all the way to 1500 (for visible) and 2184 (for NIR). See the two graphs below.

We obviously want to ignore distributions whose tails stretch that far, and I think homing in on the finer details of the distribution from 1-15 is more telling since volume pricing is offered in 1, 6, and 11 pieces. The Visible LD Graphs and NIR LD Graphs tabs show more histograms and a bar plot to see the quantities purchased in this range.

The bar plots indicate that volume pricing is most popular. LDs sold in quantities of 1 are the most popular, usually followed by the "Other" category, which encompasses quantities that are >1 but <6 as well as >6 but <11; basically, quantities that are between volume pricing batches.
