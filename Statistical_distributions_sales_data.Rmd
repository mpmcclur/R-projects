...
This report, which implements Shiny API functionality, reviews quantity sold data for two types of laser diodes (LDs) at a company: one dataset consists of LDs in the visible wavelength range, while the other dataset is the near-infrared (NIR) range. Included are histograms to show distributions as well as bar plots to show which batches are most popular (e.g., is volume pricing for 6 more popular than volume pricing for 11?).

First, we look at the full distribution of each dataset, which is left skewed. However, viewing the full distribution means we also see the outliers, which stretch all the way to 1500 (for visible) and 2184 (for NIR). See the two graphs below.

We obviously want to ignore the distribution tail that stretches out that far, and I think honing in on the finer details of the distribution from 1-15 quantities sold is more telling, since volume pricing is offered in 1, 6, and 11 pieces. These  tabs show more histograms as well as a bar plot to see the quantities purchased in this range. The Visible LD Graphs and NIR LD Graphs tabs show more histograms as well as a bar plot to see the quantities purchased in this range.

I think the bar plots are particularly telling about which volume pricing option is most popular. LDs sold in quantities of 1 are the most popular, usually followed by the "Other" category, which encompasses quantities that are >1 but <6 as well as >6 but less <11; basically, quantities that are in-between the volume pricing batches.

<b>Note</b>: Since these data are proprietary, only the code and text are shown in this report.
...
Below are individual histograms (organized in a lattice form to display all item #s) showing the distribution of LD quantities sold from 1 to 15. Note that item #s L405G2, L450G1, L515A1, and L785H1 offer volume pricing for 1, 10, 20, 51, and 101 pieces.
...
```
<b>Individual Histograms tab:</b><br />
Below are individual histograms showing the full distribution of LD quantities sold, limited from 1 to 15. If outliers are included, then there will only be one large bar that spans 0-50, which doesn't provide enough detail. Note that item #s L405G2, L450G1, L515A1, and L785H1 offer volume pricing for 1, 10, 20, 51, and 101 pieces.
