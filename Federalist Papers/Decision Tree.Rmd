...
```{r setup}
packages <- c("C50","caret","rpart","e1071","rattle","rpart.plot","prediction","spls")
...
The model will be created using recursive partitioning, or the rpart function.
```{r Data Preparation}
...
#The probabilities are shown for all the authors. Here, we see that, aside from disputed paper #3, all papers are most likely (most probably) authored by Madison. There is a small probability that both Hamilton and Madison authored the papers, and is it improbable, except for paper #3, that Hamilton is the author (!), meaning Disputed Paper #3 is most likely authored by Jay, but we will see if this holds up in pre-pruning and post-pruning.
```
Instead of model tuning, we will go straight to model pre-pruning in order to increase the accuracy. Pre-pruning can be performed by adding more parameters to the rpart function. This is to prevent the model overfitting the tree and increasing its complexity. The minsplit, minbucket, and maxdepth parameters are very influential here.
...
Prior to using the rpart function, I created the model using the train function. However, I abandoned this function in favor of rpart because train was not providing meaningful or easily verifiable predictive accuracy.
